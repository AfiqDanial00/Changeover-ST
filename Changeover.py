...
def format_time_am_pm(time_obj):
    ...
    if time_obj and isinstance(time_obj, time):
...
def save_data(df):
    """Save data to CSV with AM/PM format"""
    csv_df = df.copy()
    
    # Convert times to AM/PM format for CSV
    if 'Start_Time' in csv_df.columns:
        csv_df['Start_Time'] = csv_df['Start_Time'].apply(
            lambda x: format_time_am_pm(x) if pd.notnull(x) else ""
        )
    if 'End_Time' in csv_df.columns:
        csv_df['End_Time'] = csv_df['End_Time'].apply(
            lambda x: format_time_am_pm(x) if pd.notnull(x) else ""
        )
...
def main():
    ...
    default_time_started = "8:00 AM"
    default_time_completed = "8:30 AM"
    default_product_from = ""
    default_product_to = ""
    ...
    with col1:
        with st.expander(f"### {t['changeover_details']}", expanded=True):
            date = st.date_input(t['date'], value=default_date)
            time_started_str = st.text_input(t['time_started'], value=default_time_started)
            time_completed_str = st.text_input(t['time_completed'], value=default_time_completed)
            product_from = st.text_input(t['product_from'], value=default_product_from)
            product_to = st.text_input(t['product_to'], value=default_product_to)
            operator_name = st.text_input(t['operator'], value=default_operator_name)

    with col2:
        with st.expander(f"### {t['documentation']}", expanded=True):
            remarks = st.text_area(t['remarks'], value=default_remarks, height=100, placeholder=t['remarks_placeholder'])
    ...
    if st.button(f"✅ {t['submit']}", use_container_width=True):
        ...
        # Create new record with proper null handling
        new_data = {
            "Date": [date.strftime('%Y-%m-%d')],
            "Start_Time": [time_started] if time_started else [None],
            "End_Time": [time_completed] if time_completed else [None],
            "Duration_Minutes": [duration_minutes] if duration_minutes is not None else [None],
            "From_Part": [product_from] if product_from else [None],
            "To_Part": [product_to] if product_to else [None],
            "Operator": [operator_name] if operator_name else [None],
    ...
    if not df.empty:
        # Prepare data for download with AM/PM formatting
        download_df = df.copy()
        download_df['Start_Time'] = download_df['Start_Time'].apply(
            lambda x: format_time_am_pm(x) if pd.notnull(x) else ""
        )
        download_df['End_Time'] = download_df['End_Time'].apply(
            lambda x: format_time_am_pm(x) if pd.notnull(x) else ""
        )
